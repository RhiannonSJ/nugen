...
namespace evg {
  class AddGenieEventsToLAr;

  struct AddGenieEventsToLArParams {
    ...
    Sequence<std::string> fileList {
      Name("fileList"),
      Comment("list of input gntp.*.ghep.root files or .list file containing locations of root files"),
  ...
  }; // AddGenieEventsToLArParams
}

class evg::AddGenieEventsToLAr : public art::EDProducer {
public:

  // Allow 'art --print-description' to work
  using Parameters = art::EDProducer::Table<evg::AddGenieEventsToLArParams>;

  //explicit AddGenieEventsToLAr(fhicl::ParameterSet const & p);
  explicit AddGenieEventsToLAr(const Parameters & p);
  ...
  AddGenieEventsToLAr(AddGenieEventsToLAr const &) = delete;
  AddGenieEventsToLAr(AddGenieEventsToLAr &&) = delete;
  AddGenieEventsToLAr & operator = (AddGenieEventsToLAr const &) = delete;
  AddGenieEventsToLAr & operator = (AddGenieEventsToLAr &&) = delete;
...
private:
  ...
  // Subrun info
  bool fFilesAsList;        // Whether the filelist has been passed by a wildcard or .list file
  int fTotInteractions;     // Total number of interactions available
  int fGoodInteractions;    // Number of interactions converted to art::Events
  float fPOT;               // Total POT from summed gtree->GetWeight()
  float fGoodPOT;           // POT weighted by the number of interactions converted to all interactions

};

evg::AddGenieEventsToLAr::AddGenieEventsToLAr(const Parameters& params)
  : EDProducer(params)
  , fParams(params)
  , fGlobalTimeOffset(0)
  , fTimeShifter(0)
  ...
  ,fGoodPOT(0)
{
  ...
  mf::LogDebug("AddGenieEventsToLAr")
  ...
  // Check for fileList type and fill vector properly if necessary
  TString tFileList(fFileList[0].c_str());
  if(tFileList.Contains(".list")){
    fFilesAsList = true;
    std::string fileListName = fFileList[0];
    fFileList.clear();
    std::ifstream infile(fileListName.c_str());
    std::string line;
    while (std::getline(infile, line))
    {
      fFileList.push_back(line);
      std::cout << " Pushed back file named: " << line << " to the file list" << std::endl;
    }
    std::cout << "Filled fFileList with all file names, vector now has: " << fFileList.size() << " entries " << std::endl;
  }
  ...
  for (size_t i=0; i < fFileList.size(); ++i) {
    ...
    // Get the gtree from the current file to calculate the event weight correctly
    if(fFilesAsList){
      TFile fTemp(fFileList[i].c_str(),"READ");
      TTree *tTemp = static_cast<TTree*>(fTemp.Get("gtree"));
      fPOT += tTemp->GetWeight();
      std::cout << " POT is currently: " << fPOT << " having added " << tTemp->GetWeight() << " from the current file " << std::endl;
    }
    else{
      fPOT += fGTreeChain->GetWeight();
      std::cout << " POT is currently: " << fPOT << " having added " << fGTreeChain->GetWeight() << " from the current file " << std::endl;
    }
  }
  mf::LogDebug("AddGenieEventsToLAr") << outFileList;
  ...
  **/
  ...
  while ( ( obj = next() ) ) {
    ...
    if ( ! belement && ! bobject ) {
      ...
      mf::LogError("AddGenieEventsToLAr")
  ...
  mf::LogInfo("AddGenieEventsToLAr")
    << fMyModuleLabel
    << " (" << fMyModuleType << ") "
  ...
  if ( fOutputPrintLevel > 0 ) {
    if ( fOutputDumpFileName == ""          ||
      ...
      mf::LogDebug("AddGenieEventToLAr")
        << "#### AddGenieEventsToLAr::ctor open "
        << fOutputDumpFileName
        << std::endl << std::flush;
      fOutputStream =
...
evg::AddGenieEventsToLAr::~AddGenieEventsToLAr()
{
  // release resources
  ...
  if ( fOutputStream && ( fOutputDumpFileName != "std::cout" ) ) {
    ...
    if ( ofs ) {
      mf::LogDebug("AddGenieEventToLAr")
        << "#### AddGenieEventsToLAr::dtor close "
        << fOutputDumpFileName
        << std::endl << std::flush;
      ofs->flush();
      ofs->close();
...
void evg::AddGenieEventsToLAr::produce(art::Event & evt)
{

  //std::cerr << "AddGenieEventsToLAr::produce start" << std::endl << std::flush;
  ...
  mf::LogDebug("AddGeniEventsToLAr") << "#### AddGenieEventsToLAr::produce "
  ...
  while ( entries.size() != n ) {
    if ( ! fRandomEntries ) {
      ...
      if ( std::find(entries.begin(),entries.end(),indx) != entries.end() ) {
        // mf::LogInfo("AddGeniEventsToLAr") << "rejecting "
  ...
  //mf::LogInfo("AddGeniEventsToLAr") << "entries.size " << entries.size()
  //                                  << " " << msg.str();
  ...
  for (size_t i=0; i<n; ++i) {
    ...
    /*
    mf::LogInfo("AddGenieEventsToLAr")
      << "#### AddGenieEventsToLAr::produce " << i+1 << " of " << n
      << " using entry " << ientry
      << std::endl;
    */
    ...
    if ( fOutputStream ) {
      ...
      //std::cerr << "####  AddGenieEventsToLAr::produce() writing to "
      //          << fOutputDumpFileName
      //          << std::endl << std::flush;
    ...
    if ( fAddMCFlux ) {
      if ( fGNuMIFluxPassThroughInfo ) {
        ...
        TLorentzVector pos(fGSimpleNtpEntry->vtxx,fGSimpleNtpEntry->vtxy,fGSimpleNtpEntry->vtxz,0);
        mcflux.fgen2vtx = (pos.Vect()-(*grec->Vertex()).Vect()).Mag();
  ...
  //std::cerr << "AddGenieEventsToLAr::produce put into event"
  //          << std::endl << std::flush;

  fGoodInteractions += mctruthcol->size();
...
//-------------------------------------------------------------------------
void evg::AddGenieEventsToLAr::ParseCountConfig()
{
...
size_t evg::AddGenieEventsToLAr::GetNumToAdd() const
{
...
//-------------------------------------------------------------------------
void evg::AddGenieEventsToLAr::ParseTimeConfig()
{
...
//-------------------------------------------------------------------------
void evg::AddGenieEventsToLAr::ParseVtxOffsetConfig()
{
...
void evg::AddGenieEventsToLAr::beginSubRun(art::SubRun & sr)
{
  // Implementation of optional member function here.
}

void evg::AddGenieEventsToLAr::endSubRun(art::SubRun & sr)
{
  // Implementation of optional member function here.
  auto p = std::make_unique<sumdata::POTSummary>();

  fTotInteractions = fNumMCRec;
  fGoodPOT = (fGoodInteractions/static_cast<float> (fTotInteractions)) * fPOT;

  std::cout << " Total interactions: " << fTotInteractions << std::endl;
  std::cout << " Good interactions:  " << fGoodInteractions << std::endl;
  std::cout << " Total POT:          " << fPOT << std::endl;
  std::cout << " Good POT:           " << fGoodPOT << std::endl;

  p->totpot     = fPOT; 
  p->totgoodpot = fGoodPOT;

  sr.put(std::move(p), art::fullSubRun());

  return;
}

void evg::AddGenieEventsToLAr::beginJob()
{
  // Implementation of optional member function here.
  //fPOT += fGTreeChain->GetWeight();
  //std::cout << " Current weight: " << fGTreeChain->GetWeight() << std::endl;
...
/*
void evg::AddGenieEventsToLAr::reconfigure(const Parameters & params)
{
  fParams = params;
}
*/
...
/*
void evg::AddGenieEventsToLAr::beginRun(art::Run & r)
{
  // Implementation of optional member function here.
}

void evg::AddGenieEventsToLAr::endRun(art::Run & r)
{
  // Implementation of optional member function here.
}

void evg::AddGenieEventsToLAr::reconfigure(fhicl::ParameterSet const & p)
{
  // Implementation of optional member function here.
  std::cerr << "evg::AddGenieEventsToLAr::reconfigure() not implemented"
            << std::endl;
  abort();
}

void evg::AddGenieEventsToLAr::respondToCloseInputFile(art::FileBlock const & fb)
{
  // Implementation of optional member function here.
}

void evg::AddGenieEventsToLAr::respondToCloseOutputFiles(art::FileBlock const & fb)
{
  // Implementation of optional member function here.
}

void evg::AddGenieEventsToLAr::respondToOpenInputFile(art::FileBlock const & fb)
{
  // Implementation of optional member function here.
}

void evg::AddGenieEventsToLAr::respondToOpenOutputFiles(art::FileBlock const & fb)
{
  // Implementation of optional member function here.
}
*/

DEFINE_ART_MODULE(evg::AddGenieEventsToLAr)
